...
module.exports = function(RED) {
  function get_conferences(config) {
    ...
    node.on('input', async (msg, send, done) => {
      ...
      const params = config.expand ? '?expand=participants' : '';
      try {
        const response = await fetch(`${url}/v1/Accounts/${accountSid}/Conferences${params}`, {
          method: 'GET',
        ...
        if (err.statusCode) {
          node.error(`GetConferences failed with ${err.statusCode}`);
          msg.statusCode = err.statusCode;
          msg.errorMessage = err.statusText;
        } else {
          const errorMessage = `Error getting conferences ${err.message}`;
  ...
  RED.nodes.registerType('get_conferences', get_conferences);
};
