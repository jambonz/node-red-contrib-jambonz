...
module.exports = function(RED) {
  function get_queues(config) {
    ...
    node.on('input', async (msg, send, done) => {
      ...
      const search = await new_resolve(RED, config.search, config.searchType, node, msg);
      const params = search ? `?search=${encodeURIComponent(search)}` : '';
      try {
        const response = await fetch(`${url}/v1/Accounts/${accountSid}/Queues${params}`, {
          method: 'GET',
        ...
        if (err.statusCode) {
          node.error(`GetQueues failed with ${err.statusCode}`);
          msg.statusCode = err.statusCode;
          msg.errorMessage = err.statusText;
        } else {
          const errorMessage = `Error getting queues ${err.message}`;
  ...
  RED.nodes.registerType('get_queues', get_queues);
};
